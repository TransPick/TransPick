...
namespace TransPick.Highlighters
{
	internal class CrossHair : IDisposable
	{
		...
		private void DrawGraphics(object sender, DrawGraphicsEventArgs e)
		{
			...
			gfx.DashedLine(_brushes["red"], Display.GetLeft(), cursorPoint.Y, cursorPoint.X-1, cursorPoint.Y, 1.0f);
			gfx.DashedLine(_brushes["red"], cursorPoint.X+1, cursorPoint.Y, Display.GetRight(), cursorPoint.Y, 1.0f);

			// Draw vertical Top/Bottom line.
			gfx.DashedLine(_brushes["blue"], cursorPoint.X, Display.GetTop(), cursorPoint.X, cursorPoint.Y-1, 1.0f);
			gfx.DashedLine(_brushes["blue"], cursorPoint.X, cursorPoint.Y+1, cursorPoint.X, Display.GetBottom(), 1.0f);
		}
