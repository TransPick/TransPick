...
namespace TransPick.Utilities
{
    internal class HotKeyRegister : IMessageFilter, IDisposable
    {
        ...
        internal static KeyModifier GetModifiers(Keys keydata, out Keys key)
        {
            key = keydata;
            KeyModifier modifers = KeyModifier.None;

            // Check whether the keydata contains the CTRL modifier key.
            // The value of Keys.Control is 131072.
            if ((keydata & Keys.Control) == Keys.Control)
            {
                modifers |= KeyModifier.Control;
        ...
        internal KeyModifier Modifiers { get; private set; }
        ...
        internal HotKeyRegister(IntPtr handle, int id, KeyModifier modifiers, Keys key)
        {
            if (key == Keys.None || modifiers == KeyModifier.None)
            {
